...
{ 
    Write-Error "This module requires Az.Synapse version $minVer. An earlier version of Az.Synapse is imported in the current PowerShell session. Please open a new session before importing this module. This error could indicate that multiple incompatible versions of the Azure PowerShell cmdlets are installed on your system. Please see https://aka.ms/azps-version-error for troubleshooting information." -ErrorAction Stop 
} 
elseif ($null -eq $module) 
{ 
    Import-Module Az.Synapse -MinimumVersion "$minVer" -Scope Global
}
